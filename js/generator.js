...
function getMagicItemSources() {
  const config = state.supplement?.restrictions?.global?.magicItems;
  if (!config || config.enabled === false) return [];

  const raw = config.source ?? config.sources ?? [];
...
function magicItemSourceId(value) {
  ...
  return MAGIC_ITEM_SOURCE_ALIASES[key] || key
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9-]+/g, "-")
    .replace(/^-|-$/g, "");
...
async function loadMagicItemSource(source) {
  ...
  return normalizeMagicItems(raw);
}
...
async function loadMagicItems(armyId) {
  ...
  try {
    const configuredSources = getMagicItemSources();

    // If the supplement explicitly declares sources, use them.
    // Otherwise preserve the original V4 behaviour: one file named after the army.
    ...
    for (const source of sources) {
      try {
        const items = await loadMagicItemSource(source);
        if (!items) continue;
        ...
        Object.entries(items).forEach(([category, categoryItems]) => {
          ...
          for (const item of categoryItems) {
            ...
            if (!merged[category].some(existing =>
              String(existing.id || existing.name || "") === key
            )) {
        ...
        console.log(
          `[MagicItems] Source chargée : ${source} → ${magicItemSourceId(source)}.json`
        );
      } catch (e) {
        // One unavailable source must not prevent the generator from loading.
        console.warn(
          `[MagicItems] Source indisponible : ${source}`,
          e
        );
      }
    }
    ...
    // Magic items are optional: the army can still be built.
    console.warn("[MagicItems] Chargement impossible", e);
    state.magicItems = null;
  } finally {
